...
export function validatePlayCard(
  ...
  /* ================= SILENCE ================= */
  const isSilenced = player.statuses.some(
    s => s.type === "SILENCE"
  );
  if (isSilenced) {
  ...
  // Target legality (e.g. UNTARGETABLE) is handled
  // in gameService.playCard where targetIndex is known
}
